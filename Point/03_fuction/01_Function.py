...
# # 位置参数,
# 传入的参数位置不能乱传位置，对位置敏感，少传参数
# greet('李四','女',18,189)



# # -----------------------------关键字参数------------
# 官方的标准是关键字参数赋值时=左右两端不加空格
...
# 这个参数也会变成 可选参数
# 带默认值的函数的默认值参数必须放在最后，或者说带默认值的参数之后都是默认值参数
...
# ----------------------可变参数--------------------
# 分为1. 可变位置参数 2. 可变关键字参数

# 可变位置参数，在定义时，在形参前面加上*，可以接收任意数量的位置参数，并打包成一个   元组
# 习惯上在可变位置参数为 *args   arguments(参数)
def test1(*args):
    print(args)
    pass
test1('张三','男',18,172)
...
# 可变关键字参数，在定义时，在形参前面加上两个**，可以接收任意数量的关键字参数，并打包成一个 ---> 字典
def test2(**kwargs):
    print(kwargs)
    pass
